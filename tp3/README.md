...
En `dataset-folder/` tenemos los datos extra con los que complementamos el dataset entregado(`extra.csv`).

### Otros directorios

En `src/` está el código de C++, en particular en `src/metnum.cpp` está el entry-point de pybind.

En `notebooks/` estan los resultados y ejecuciones de la experimentacion, asi como el notebook para modificar el dataset extra y acoplarlo a los datos(`Union de datos extra a dataset.ipynb`).
...
## Compilación
Ejecutar la primera celda del notebook `Analisis de regresores.ipynb` o seguir los siguientes pasos:
